...
from data.dataset_base_depth import DataConfig, PackedDataset, collate_wrapper
...
class TrainingArguments:
    ...
    total_steps: int = field(
        default=5000, metadata={"help": "Total number of optimizer steps to train for."}
    )
    ...
    # --- module freezing ---
    train_only_generator: bool = field(
        default=False,
        metadata={"help": "Only train diffusion generator and not lm."},
    )
    train_only_llm2vae: bool = field(
        default=False,
        metadata={"help": "Only train llm2vae and not generator."},
    )
    freeze_llm: str = field(
        default="not",
        metadata={"help": "('not', 'all', 'not_gen', 'not_llm2vae')"},
    )
...
def main():
    ...
    if training_args.freeze_llm in ["not", "all", "not_gen", "not_llm2vae"]:
        if training_args.freeze_llm == "not":
            pass
        elif training_args.freeze_llm in ["all", "not_gen", "not_llm2vae"]:
            for param in model.language_model.parameters():
                param.requires_grad = False
            if training_args.freeze_llm == "not_gen":
                for name, param in model.language_model.named_parameters():
                    if "connector" in name or "vae2llm" in name:
                        param.requires_grad = False
                for name, param in model.language_model.named_parameters():
                    if "moe_gen" in name:
                        param.requires_grad = True
            elif training_args.freeze_llm == "not_llm2vae":
                for name, param in model.language_model.named_parameters():
                    if "connector" in name or "vae2llm" in name:
                        param.requires_grad = False
                for name, param in model.language_model.named_parameters():
        ...
        raise ValueError("Invalid value for freeze_llm.")
    ...
    fsdp_config = FSDPConfig(
        ...
        use_orig_params=(
            True if training_args.freeze_llm in ["not_gen", "not_llm2vae"] else False
        ),
    )
    print(f"use_orig_params: {fsdp_config.use_orig_params}")
