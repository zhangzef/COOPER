...
def process_trajectory_summary(summary: str):
    text, *img_objs = step
...
def process_file(path: Path):
    ...
    think_block = (
        "<think>"
        + obj["trajectory_summary"].split("\n\nANSWER")[0].strip()
        + "</think>"
    )

    # ------ answer ------
    ans_txt = obj["answer"].replace("ANSWER:", "").strip()
    instruction_list = [q_text, think_block, ans_txt]
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--json_dir",
        default="../../outputs/mathv360k-20250422-reaoning-step-40k",
        type=Path,
    )
    ap.add_argument(
        "--out_dir",
        default="./datasets/mathv360k-20250422-reaoning-step-40k-parquet-text",
        type=Path,
    )
    ap.add_argument("--rows_per_file", type=int, default=1000)
    ap.add_argument(
        "--threads", type=int, default=os.cpu_count(), help="线程数 (默认=CPU 核心数)"
    )
    args = ap.parse_args()
