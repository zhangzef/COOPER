...
class MaxLongEdgeMinShortEdgeResize(torch.nn.Module):
    ...
    def forward(self, img, img_num=1, interpolation=InterpolationMode.BICUBIC):
        """
        Args:
        ...
        return F.resize(
            img, (new_height, new_width), interpolation, antialias=self.antialias
        )
...
class DepthImageTransform:
    ...
    def __call__(self, img, is_rgb=False, is_training=True, dataset="vkitti"):
        if is_rgb:
            ...
            if dataset in [
                ...
                "hypersim",
                "nyu",
            ]:  # training with normalization depth image
                self.min_depth, self.max_depth = self.get_depth_range(dataset)
                self.set_scale_factor(dataset)
                img = torch.tensor(np.array(img), dtype=torch.get_default_dtype())

                if is_training:
                    # Scale depth image with dataset scale factor from Marigold
                    img = (
                        (img.squeeze() / self.scale_factor).unsqueeze(0).repeat(3, 1, 1)
                    )  # (Channel, Height, Width)

                    img = img.clamp(self.min_depth, self.max_depth)

                    img = (img - self.min_depth) / (
                        self.max_depth - self.min_depth
                    )  # convert to range [0, 1.0]

                    # normalize depth image to the bagel's numerical range
                    img = self.normalize_transform(img)
                    return img
                else:  # inference/evaluation with raw depth image
                    # Scale depth image with dataset scale factor from Marigold
                    img = img.squeeze() / self.scale_factor  # (Height, Width)

                    valid_mask = self.get_valid_mask(img, dataset, is_training)

                    return (img, valid_mask)
            elif dataset in ["segmentation"]:
                img = self.resize_transform(
                    img, img_num=1, interpolation=InterpolationMode.NEAREST
                )
                if is_training:
                    img = (
                        torch.tensor(np.array(img), dtype=torch.get_default_dtype())
                        .unsqueeze(0)
                        .repeat(3, 1, 1)
                    )
                    img = self.normalize_transform(img)
    ...
    def get_valid_mask(self, img, dataset, is_training=True):
        valid_mask = torch.logical_and(
            img > self.min_depth, img < self.max_depth
        ).bool()
        if not is_training:
            eval_mask = torch.zeros_like(valid_mask).bool()
            gt_height, gt_width = valid_mask.shape
            if dataset in ["vkitti", "kitti"]:
                if len(img.shape) == 3:
                    eval_mask[
                        :,
    ...
    def set_scale_factor(self, dataset):
        # scale factor from Marigold
        assert dataset in ["vkitti", "nyu", "kitti", "hypersim"]
        if dataset == "vkitti":
    ...
    @staticmethod
    def get_depth_range(dataset):
        # depth range from Marigold
        assert dataset in ["vkitti", "nyu", "kitti", "hypersim"]
        min_depth = 1e-5
        max_depth = 80.0
        if dataset == "vkitti":
    ...
    @staticmethod
    def kitti_benchmark_crop_tensor(input_img):
        """
    ...
    def kitti_benchmark_crop(input_img):
        ...
        # 获取输入图像的尺寸
        width, height = input_img.size  # PIL.Image.size返回(width, height)
        ...
        # 计算右下角坐标
        right_margin = left_margin + KB_CROP_WIDTH
        bottom_margin = top_margin + KB_CROP_HEIGHT

        # 裁剪图像，PIL的crop方法接受(left, upper, right, lower)元组
        out = input_img.crop((left_margin, top_margin, right_margin, bottom_margin))
